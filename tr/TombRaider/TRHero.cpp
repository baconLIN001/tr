...
void TRHero::beAttacked(int dmg){
    if(arm < dmg){
        hp -= dmg - arm;
        gBgm->playMusic("Resources/Bgm/hero受伤.wav",true);
    }
    if(hp <= 0){
        hp = 0;
        goDie();
...
TRBullet* TRHero::fire(){
    if(bullet_cd_rem <= 0 && !flgdebuff_nogun){
        ...
        bul -> startMoving();
        gBgm->playMusic("Resources/Bgm/激光.wav",true);
        lock(fire_cd);
        return bul;
    }
...
void TRHero::linkWalkingAnimator(){
    if(isWalkingAnimated()){
        gBgm->playMusic("Resources/Bgm/走路.wav",true);
...
//set low speed in the water
void TRHero::setLowVelocity(int lowVelocity)
{
    int temp = vel;
...
//draw the blood bar
void TRHero::drawHpBar(SDL_Surface* s,string path,int x,int y,int h,int w)
{
    if(this->hp<=0)
        return;
    SDL_Rect r2 = this->getBoxRect();
    SDL_Rect r;
    r.x = r2.x - x;
    r.y = r2.y - y;
    r.h = 4;
    r.w = this->getHP()*15/500;
    SDL_FillRect(s,&r,SDL_MapRGB(s->format,200,0,0));
}
